...
> Illustrative layout — **adjust to your actual tree.**

```
.
├── data/
│   ├── manifests/              # CSVs: paths, labels, focal weights, U-Ones/U-Ignore columns
│   └── README.md               # how to obtain NIH + CheXpert
├── notebooks/
│   ├── 01_eda_nih.ipynb
│   ├── 02_eda_chexpert.ipynb
│   ├── 03_preprocessing.ipynb  # Center Crop vs Zero-Padding
│   ├── 10_m1_nih_baseline.ipynb
│   ├── 11_m2_domain_transfer_uignore.ipynb
│   ├── 12_m3_uones.ipynb
│   ├── 13_m4_hybrid.ipynb
│   └── 14_interpretability_gradcam_medsam.ipynb
├── src/
│   ├── model.py                # Xception + head
│   ├── losses.py               # Focal Loss + per-label weighting
│   ├── data.py                 # loaders, U-Ones/U-Ignore masking
│   ├── train.py                # 3-phase progressive fine-tuning
│   └── eval.py                 # AUROC/AUPRC/Brier, DeLong, bootstrap CIs
├── interpretability/
│   ├── gradcam.py
│   └── medsam_pipeline.py
├── app/                        # CXR Analyzer (Gradio)
├── results/
│   └── ablation_metrics_final.json
├── requirements.txt
└── README.md
```
...
```bash
# clone
git clone https://github.com/Pablo-Henrique-IA/<repo-name>.git
cd <repo-name>
...
```bash
# 1) prepare images (NIH resize 1024→512; CheXpert Center Crop → 512)
python src/data.py --build-manifests

# 2) reproduce the ablation
python src/train.py --model m1     # NIH baseline
python src/train.py --model m2     # domain transfer + U-Ignore  (publication model)
python src/train.py --model m3     # U-Ones, ε=0.05
python src/train.py --model m4     # hybrid, ε=0.15 on pleural_effusion + pneumothorax

# 3) evaluate (AUROC/AUPRC/Brier, DeLong, bootstrap CIs)
python src/eval.py --checkpoint <path> --split chexpert_val

# 4) launch the interpretability app
python app/cxr_analyzer.py
```

<!-- TODO: pin exact versions in requirements.txt. Core deps: tensorflow/keras (classifier),
     torch + segment-anything/MedSAM (segmentation), scikit-learn, numpy, pandas, opencv-python,
     matplotlib, gradio. -->
